...
def checkProcesses(popenDict):
	...
	for popen in popenDict:
		...
		if retcode:
			raise Exception("Command '{cmd}' failed with return code {retcode}. Log files are {stdout} and {stderr}.\n\n {stdout} contains {stdout_text}.\n\n {stderr} contains {stderr_text}.\n\n".format(cmd=cmd,retcode=retcode,stdout=stdout.name,stderr=stderr.name,stdout_text=stdout_text,stderr_text=stderr_text))
