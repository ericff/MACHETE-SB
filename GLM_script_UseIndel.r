...
## if anomalies don't exist, use the LINEAR glm
anomalies.exist=0
if (dim(good_anomaly_reads)[2]==dim(bad_anomaly_reads)[2] &
dim(good_anomaly_reads)[1]*dim(bad_anomaly_reads)[1]>0){
    anomalies.exist=1
    all_anomaly_reads=rbind(good_anomaly_reads,bad_anomaly_reads)
...
## need to test if AnomalyGLM and AnomalyjunctioNPredictions exists later

if (anomalies.exist==0){
print ("USING GLM MODEL for INDEL MODEL")
ANOMALYsaves=linearDecoyGLMoutput[[1]]
ANOMALYjunctionPredictions=linearDecoyGLMoutput[[2]]
AnomalyGLM = linearDecoyGLM
}
...
## making change on apr 11 2017 b/c of problem with bad_anomaly_reads
##  and good_anomaly_reads not matching when bad has dim 0 12
if (anomalies.exist==1){
